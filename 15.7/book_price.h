...
class LimitedDiscountPrice : public BookPrice
{
public:
    LimitedDiscountPrice() = default;
    LimitedDiscountPrice(const std::string &Id, double Price, size_t NoDiscountAmount, double Discount);
    virtual ~LimitedDiscountPrice() = default;
public:
    double getDiscount() const { return m_Discount; }
    virtual double getTotalPrice(size_t Number) const override;
private:
    size_t m_NoDiscountAmount;
    double m_Discount;
};
