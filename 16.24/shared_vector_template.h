...
public:
    ...
    SharedVector(const Itr& beg, const Itr& end);
...
template <typename T>
template <typename Itr>
SharedVector<T>::SharedVector(const Itr& beg, const Itr& end)
    : m_SharedVectorImpl{std::make_shared<std::vector<T>>(beg, end)}
{
}
