...
public:
    ...
    T* end() const { return m_FirstFree; }

    size_t size() const { return (m_FirstFree-m_First); }
    size_t capacity() const { return (m_Last-m_First); }
...
{
    checkMemory();
    m_Allocator.construct(m_FirstFree, std::forward<Args>(args)...);
    m_FirstFree++;

    std::cout << "A T is emplaced-back." << std::endl;
    std::cout << "Size: " << size() << ", capacity: " << capacity() << std::endl;
}
